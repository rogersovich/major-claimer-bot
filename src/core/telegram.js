...
export class Telegram {
  ...
  constructor() {
    ...
    this.url = "https://major.glados.app";
    this.bot = "Major";
  }
  ...
  async queryModificaiton() {
    try {
      ...
      if (queryAccountList[choice - 1] != undefined) {
        ...
        let oldQuery = Helper.readQueryFile(
          `${this.accountName}/query.txt`
        )
        const ctx2 = `Old Query :\n ${oldQuery}\n \nPlease Enter New Token Only :`;

        const newToken = await input.text(ctx2);
        
        // Replace old token using a regular expression
  ...
  async queryCreation() {
    try {
      ...
      // let ctx3 = "Please Enter Telegram ID : ";
      // const tele_id = await input.text(ctx3);
      // const full_query = `token: ${query}, account_id: ${tele_id}`

      Helper.saveQueryFile(this.accountName, query);
